...
metadata:
  hermes:
    tags: [paperclip, tasks, coordination, infrastructure]
    category: platform
...
Сервисы доступные внутри Docker-сети:

| Сервис | Хост:порт | Протокол | Назначение |
|--------|-----------|----------|------------|
| paperclip-server | `paperclip-server:3100` | HTTP REST API | Paperclip control plane (порт 3100, НЕ 3000) |
| paperclip-mcp | `paperclip-mcp:8082/mcp` | MCP StreamableHTTP | MCP-прокси к Paperclip API |
| paperclip-db | `paperclip-db:5432` | PostgreSQL | БД Paperclip (user: `paperclip`, password: `paperclip`) |
| rag-mcp | `rag-mcp:8081/mcp` | MCP StreamableHTTP | RAG-поиск (Outline, Nextcloud) |
| Outline | `outline.collaborationism.tech` | HTTPS REST API | База знаний |

**Правила доступа:**
- Основной способ работы с Paperclip — MCP-инструменты (`mcp_paperclip_paperclip_*`). НЕ проверяй доступность через curl/HTTP — используй MCP.
- paperclip-server слушает на порту **3100** (не 3000). Если нужен прямой HTTP-запрос — используй `http://paperclip-server:3100`.
- Прямое подключение к PostgreSQL (`paperclip-db:5432`) — только fallback если MCP недоступен. Для подключения: `psql -h paperclip-db -U paperclip -d paperclip`.
- НЕ пиши в БД напрямую — только чтение для диагностики. Все изменения через MCP или REST API.

**Диагностика MCP-проблем:**
1. Если `mcp_paperclip_paperclip_*` tools не отвечают — это не значит что Paperclip упал. MCP-соединение могло таймаутнуться.
2. Не трать время на curl-диагностику портов — сразу fallback на другой способ доступа.
3. Если MCP и REST API оба недоступны — используй PostgreSQL для чтения статуса задач.
