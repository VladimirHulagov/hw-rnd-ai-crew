...
class SkillGitSync:
    def __init__(self, repo_url: str, branch: str, path: str, token: str, author: str):
    ...
    def _git_env(self) -> dict:
        return {
            "GIT_AUTHOR_NAME": self.author.split("<")[0].strip(),
            "GIT_AUTHOR_EMAIL": self.author.split("<")[1].rstrip(">").strip() if "<" in self.author else "",
            "GIT_COMMITTER_NAME": self.author.split("<")[0].strip(),
            "GIT_COMMITTER_EMAIL": self.author.split("<")[1].rstrip(">").strip() if "<" in self.author else "",
        }
    ...
    def _ensure_repo(self) -> bool:
        ...
        if self._repo_dir is None:
            self._repo_dir = Path("/tmp/skill-git-sync-repo")
    ...
    def pull_skills(self, conn, company_id: str) -> dict:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                WHERE company_id = %s AND metadata->>'sourceKind' = 'git_sync'
            """, (company_id,))
